$(document).ready(function(){
    function submit_fx(){
        ...
        function inf_form_submit(data,callback){
            ...
            $(inf_frame.contentDocument).find('[name="inf_field_Email"]').val(data.email);
            $(inf_frame.contentDocument).find('[name="inf_custom_Age"]').val(data.age);

            if(data.gender == "male") {
                $(inf_frame.contentDocument).find('[name="inf_option_Gender"]').prop("checked",false);
                $(inf_frame.contentDocument).find('#inf_option_Gender_228').prop("checked",true);
            } else if(data.gender == "female"){
                $(inf_frame.contentDocument).find('[name="inf_option_Gender"]').prop("checked",false);
                $(inf_frame.contentDocument).find('#inf_option_Gender_230').prop("checked",true);
            }
